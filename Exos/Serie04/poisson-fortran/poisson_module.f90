...
contains
	...
	function id(width,row,col) result(idm)
		implicit none
		integer, intent(in) :: width,col,row
		integer :: idm
		idm = (width*row+col)
	end function id

	subroutine print_mat(tab, col, row)
		implicit none
		real, dimension(:), intent(in) :: tab
		integer, intent(in) :: col, row
		integer :: i,j
		do, i=1,col
			!write(*,"100g15.5") ( tab(i,j), j=1,row )
			write(*,*) tab(i)
		enddo			
